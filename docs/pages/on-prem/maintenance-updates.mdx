...
> ⚠️ Ensure you have enough hardware resources (CPU, GPU, RAM) before scaling up.
...
2. Re-authenticate to the private ECR:
   > Before updating, ensure you have the latest Authenta-issued ECR credentials. These credentials authenticate against Authenta’s private AWS Account ID and are required to pull updated images.
   ```bash
...
> 💡 Regular backups ensure quick recovery in case of system failure.
...
> ⚠️ Ensure you have backups before pruning. These commands delete unreferenced data permanently.
...
| Queue not visible in RabbitMQ | Misconfigured queue name                    | Verify `.env` → `RABBITMQ_QUEUE` matches `task_queue`       |
