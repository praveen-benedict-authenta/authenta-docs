...
| `13` | `pdf-tampering-detection` | PDF Tampering Detection | Detects if a PDF document has been tampered or forged | PDF | 300s |

## PDF Tampering Detection

Task type `13` (`pdf-tampering-detection`) accepts **PDF files only** as input. The workflow is identical to other task types — create a job, upload the PDF to the pre-signed URL, then finalize. Use `contentType: "application/pdf"` and `slotName: "original"` when creating the job.
...
The result fields depend on the selected task type.
...
`taskTypeId` is required in the JSON body as a **string**. Do not send it as a query parameter or as a number.
...
interface ImageDetectionResult {
  ...
  fakeConfidencePercent: number;
  realConfidencePercent: number;
}
```
...
For image detection, `result` is a flat object. Use `fakeConfidencePercent` and `realConfidencePercent` as percentages; do not look for `confidence` or `isAiGenerated`. If your application needs a normalized confidence value between `0` and `1`, calculate `fakeConfidencePercent / 100`.
