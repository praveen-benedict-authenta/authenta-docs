...
```bash
curl -X POST "https://platform.authenta.ai/api/v1/jobs?taskTypeId=13" \
...
The result will include a `tamperingDetected` boolean and a `confidence` score indicating the likelihood of document forgery.
