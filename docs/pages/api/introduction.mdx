...
- **GET `/api/v1/jobs`** — List All Jobs _(Query)_
- **GET `/api/v1/jobs/{id}`** — Get Job Details _(Query)_
- **POST `/api/v1/jobs`** — Create Job & Upload Media _(Mutation)_
- **POST `/api/v1/jobs/{id}/finalize`** — Finalize Job & Queue Processing _(Mutation)_
- **POST `/api/v1/jobs/{id}/cancel`** — Cancel Job _(Mutation)_
- **DELETE `/api/v1/jobs/{id}`** — Delete Job _(Mutation)_

Full details are available in [Jobs API References](/api/reference/jobs)
